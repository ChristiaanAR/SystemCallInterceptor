 ...
 * The exiting process's PID is retrieved using the current variable (current->pid).
 ...
 *      - REQUEST_STOP_MONITORING to stop monitoring for 'pid' whenever it issues 'syscall'
 ...
 *   Use the calltable_lock and pidlist_lock to ensure mutual exclusion for accesses 
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	switch(cmd){
		case 1: // REQUEST_SYSCALL_INTERCEPT
			if(syscall < 0 || syscall == MY_CUSTOM_SYSCALL || syscall > NR_syscalls) { // invalid syscall
				return -EINVAL;
			} else if(current_uid() != 0) { // not root
				return -EPERM;
			} else if(table[syscall].intercepted == 1) { // syscall already intercepted
				return -EBUSY;
			} 
		...
		case 2: // REQUEST_SYSCALL_RELEASE
			if(syscall < 0 || syscall == MY_CUSTOM_SYSCALL || syscall > NR_syscalls || table[syscall].intercepted == 0) { // invalid syscall / syscall not yet intercepted
				return -EINVAL;
			} else if(current_uid() != 0) { // not root
				return -EPERM;
			}
		...
		case 3: // REQUEST_START_MONITORING
			if(syscall < 0 || syscall == MY_CUSTOM_SYSCALL || syscall > NR_syscalls) { // invalid syscall
				return -EINVAL;
			} else if (pid < 0 || (pid != 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)) { // invalid pid
				return -EINVAL;
			} else if(current_uid() != 0 && pid == 0) { // not root
				return -EPERM;
			} else if(current_uid() != 0 && check_pid_from_list(pid, current->pid) != 0) { // pid not owned by calling process
				return -EPERM;
			} else if(check_pid_monitored(syscall, pid) == 1) { // syscall already monitored
				return -EBUSY;
			} 
			...
			if(table[syscall].monitored != 2) {
				...
				if(r == 0 && table[syscall].monitored == 0) {
		...
		case 4: // REQUEST_STOP_MONITORING
			if(syscall < 0 || syscall == MY_CUSTOM_SYSCALL || syscall > NR_syscalls) { // invalid syscall
				return -EINVAL;
			} else if (pid < 0 || (pid != 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)) { // invalid pid
				return -EINVAL;
				...
				return -EPERM;
			} else if((table[syscall].monitored != 2 && check_pid_monitored(syscall, pid) == 0) || table[syscall].intercepted == 0) { // syscall not monitored and/or intercepted
				return -EINVAL;
			} 
...
static int init_function(void) {
	...
	// initialize table
	for(cnt = 1; cnt < NR_syscalls; cnt++) {
		//table[i] = (mytable)kmalloc(sizeof(struct), GFP_KERNEL);
		table[cnt].monitored = 0;
		table[cnt].intercepted = 0;
		table[cnt].listcount = 0;
		INIT_LIST_HEAD(&(table[cnt].my_list));
	}
